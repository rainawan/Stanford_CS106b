/*
 *  A priority queue class that handles dynamic memory, organizes
 *  data, and finds the most urget priority.
 */
...
/*
 * Adds a new element to a queue, based on the elemnt's priority. The
 * higher the number, the greater the priority. First allocates enough
 * space for a new element, adds the element to the end, then compares every
 * value before elem, making necessary swaps.
 */
void PQArray::enqueue(DataPoint elem) {
    ...
    // Keep moving last priority forward until it reaches a number greater than itself
    for (int i = _numFilled-1; i > 0 && _elements[i-1].priority < _elements[i].priority; i--) {
        swapElements(i-1, i);
    }
}
...
STUDENT_TEST("time complexity enqueue and dequeue") {
    PQArray pq;

    int startSize = 500000;
    for (double n = startSize; n < 10*startSize; n*=2) {
        DataPoint i = {"", n};
        TIME_OPERATION(n, pq.enqueue(i));
    }
}
